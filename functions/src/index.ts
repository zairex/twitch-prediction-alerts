...
export const onPredictionCreate = processActionsOnPredictionCreate({
  discord_webhook: async (db, actionRef, dataRef, outcomes) => {
    const messageId = await executeDiscordWebhookActionOnCreate(actionRef[1], dataRef[1], outcomes);
    if (messageId) {
...
export const onPredictionUpdate = processActionsOnPredictionUpdate({
  discord_webhook: async (db, actionRef, dataRef, outcomes) => {
    ...
    if (alertData?.message_id) {
      await executeDiscordWebhookActionOnUpdate(actionRef[1], dataRef[1], outcomes, alertData?.message_id);
    } else {
